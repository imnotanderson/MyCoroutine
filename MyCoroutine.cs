...
public static class MyCoroutine
{
    ...
    public static int StartDelayActionUnscale(this GameObject go, float delayTime, Action action)
    {
        return go.StartContinueCoroutine(DelayAction(delayTime, action), false);
    }
    ...
    public static int StartContinueCoroutine(this GameObject go, IEnumerator ie, bool isByScaledTime = true)
    {
        if (MyCoroutineMono.IsPlaying == false) return 0;
        return MyCoroutineMono.instance.StartMyCoroutine(go, ie, isByScaledTime);
    }

    public static void StopContinueCoroutineById(this GameObject go, int id)
    {
        if (MyCoroutineMono.IsPlaying == false) return;
    ...
    class MyCoroutineMono : MonoBehaviour
    {
        ...
        struct Data
        {
            ...
            public void Upt()
            {
                ...
                if (delayTime >= 0)
                {
                    delayTime -= isByScaledTime ? Time.deltaTime : Time.unscaledDeltaTime;
                }
                if (delayTime >= 0) return;
                var moveNextResult = ie.MoveNext();
                if (moveNextResult == false)
                {
                    die = true;
                    return;
                }
        ...
        public int StartMyCoroutine(GameObject go, IEnumerator ie,bool isByScaledTime)
        {
            int id = getId();
            Data da = new Data();
            da.ie = ie;
            da.go = go;
            da.id = id;
            da.isByScaledTime = isByScaledTime;
            lock (dataBufferList)
            {
